...
def quitGame(Signal, frame):
    ...
    os.chmod(moderatorLogName, 0o0744)
...
def standardTurn():
    ...
    #**************WEREWOLVES************************
        ...
        c.broadcast('Werewolves, %s, you must choose a victim.  You have %d seconds to discuss.  Possible victims are %s'%(str(list(wolves.keys())), wolftalktime, str(sorted(list(all.keys())))), wolves)
        c.log('Werewolves debate', 0, 1, 0)
        c.allow(wolves)
        c.sleep(wolftalktime)
        c.broadcast("Werewolves, vote.", c.complement(wolves, all))
        c.broadcast('Werewolves, you must vote on a victim to eat.  You have %d seconds to vote.  Valid votes are %s.'%(wolfvotetime, str(sorted(list(all.keys())))),wolves)
        c.log('Werewolves vote', 0, 1, 0)
        wolfvote,voteType = c.poll(wolves, wolfvotetime, list(all.keys()), 'wolf', all, i['wolfUnanimous'], i['wolfSilentVote'])
    ...
    #construct the witch's options
        if len(witch) > 0 and (potions[0] or potions[1]):
            ...
            if wolfkill:
                ...
                if potions[0]:
                    witchmoves = sorted(list(all.keys())) + ['Pass']
                else:
                    witchmoves = ['Pass']
    ...
    #**************START TOWN***********************
        ...
        c.broadcast('It is day.  Everyone, %s, open your eyes.  You will have %d seconds to discuss who the werewolves are.'%(str(sorted(list(all.keys()))), towntalktime), all)
        c.log('Day-townspeople debate', 0, 1, 0)
        c.sleep(towntalktime)
        c.allow({})
        c.log('Townspeople vote', 0, 1, 0)
        c.broadcast('Townspeople, you have %d seconds to cast your votes on who to hang. Valid votes are %s'%(townvotetime, str(sorted(list(all.keys())))), all)

        killedPlayer, voteType = c.poll(all, townvotetime, list(all.keys()), 'town', all, i['townUnanimous'], i['townSilentVote'])
        if voteType == 2:
...
def listenerThread():
    ...
    while 1:
        ...
        if i == '':
            ...
            print('round %d'%round)
            print('all: %s'%str(list(all.keys())))
            print('wolves: %s'%str(list(wolves.keys())))
            wStatus = ': '
            wStatus += '%d poisons, '%potions[0]
            wStatus += '%d heals '%potions[1]
            print('witch: %s%s'%(str(list(witch.keys())), wStatus))
        elif i[0:4] == 'kill':
            player = i.split(' ')[1]
...
def main():
    ...
    if test:
        ...
        os.chmod(moderatorLogName, 0o0700)
        gameNumber = 9999
    else:
    ...
    c.log('End', 1, 1, 1)
    if not test: os.chmod('log/%dm.log'%gameNumber, 0o0744)
    if not test: os.system('echo "Game %d is over.  %s.  Please reconnect your client to play again." | wall'%(gameNumber, winner))
    exit()
