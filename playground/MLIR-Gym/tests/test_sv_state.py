...
def test_sv_state(tmp_path, globalVar, cfg_from_yaml, ctx_from_yaml):
    # write detailed state YAML
    stateCfg = StateConfig.from_yaml(globalVar.get("state_sv_config.yaml"))

    # construct engine from YAML
    engine = StateBuilder.from_config(cfg_from_yaml, ctx_from_yaml, stateCfg)
    space = engine.get_space()
    assert isinstance(space, spaces.Dict)

    obs = engine.get_observation(partial_gemm())
    assert space.contains(obs)
    # state_sv_config max_loops is set to 20, but the number of loops in the graph is 5.
    assert obs["loop_mask"].sum() == 5
